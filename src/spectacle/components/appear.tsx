...
function SteppedComponent({
  ...
  return (
    <>
      {placeholder}
      <AnimatedEl
        style={alwaysAppearActive ? (activeStyle as any) : (springStyle as any)}
        className={className}
      >
        {children}
      </AnimatedEl>
...
type SteppedComponentProps = {
  ...
  stepIndex?: number;
  children: React.ReactNode | ((step: number, isActive: boolean) => React.ReactNode);
  className?: string;
...
export function Appear({ children, ...restProps }: AppearProps) {
  return (
...
export function Stepper({
  ...
  return (
    <SteppedComponent {...restProps} numSteps={values.length} alwaysAppearActive={alwaysVisible}>
...
type StepperProps<T extends unknown[] = unknown[]> = {
  ...
  /** @deprecated use priority prop instead */
  stepIndex?: number;
  render?: (value: T[number], step: number, isActive: boolean) => React.ReactNode;
  children?: (value: T[number], step: number, isActive: boolean) => React.ReactNode;
  className?: string;
  tagName?: keyof JSX.IntrinsicElements;
  values: T;
