...
import { parse as parseQS, ParsedQuery, stringify as stringifyQS } from "query-string";
...
export function mapLocationToState(location: Pick<Location, "search">): SlideState {
  ...
  if (isNaN(nextState.slideIndex)) {
    throw new Error(`Invalid slide index in URL query string: '${queryString}'`);
  }
  ...
  if (rawStepIndex === "final") {
    ...
    if (isNaN(nextState.stepIndex)) {
      throw new Error(`Invalid step index in URL query string: '${queryString}'`);
    }
  }
